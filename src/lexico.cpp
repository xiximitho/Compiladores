...
bool Lexico::identificadorValido(gchar *str) {
  if (str[0] == '0' || str[0] == '1' || str[0] == '2' || str[0] == '3' ||
    ...
    return false;
  }  // Se o primeiro caractere da string for digito ou pontuacao o
     // identificador não é valido.
  int i;
...
bool Lexico::ehQuebra(gchar chr) { return (chr == '\n'); }
bool Lexico::ehEspaco(gchar chr) { return (chr == '\t'); }

int Lexico::ehString(gchar *str) {
  // 0 - Nenhuma aspa dupla, 1 - Somente uma aspa dupla, 2 = Duas aspas duplas
  // (String)
  short sum = 0;
       ...
       // g_print("%c%c EH OPERADOR DE ATRIBUIÇÃO\n", str[atual], str[proximo + 1]);
        analisados.push_back(novo_token(
            Token::ATRIBUICAO, Glib::ustring()
                                   ...
                                   .append(sizeof(gchar), str[proximo + 1])));
         ...
         // g_print("%c EH OPERADOR\n", str[proximo]);
          ...
          if (tmp == "<")
            ...
            analisados.push_back(
                novo_token(Token::ARITMETICO_MULTIPLICACAO, tmp));
          else if (tmp == "/")
         ...
         // g_print("%c EH CARACTERE ESPECIAL\n", str[proximo]);
          if (str[proximo] == '(')
            analisados.push_back(novo_token(
                ...
                Glib::ustring().append(sizeof(gchar), str[proximo])));
          else if (str[proximo] == ')')
            analisados.push_back(novo_token(
                Token::ESPECIAL_FECHA_PARENTESES,
                Glib::ustring().append(sizeof(gchar), str[proximo])));
        }
        proximo++;
        atual = proximo;
...
std::vector<Token> Lexico::get_tokens() { return analisados; }
...
void Token::set_token(Tipo tipo) { this->tipo_token = tipo; }

Glib::ustring Token::get_conteudo() { return this->conteudo; }

Token::~Token() = default;

Token::Tipo Token::get_tipo() { return this->tipo_token; }
